#ifndef SiOuterTracker_OuterTrackerTkMET_h
#define SiOuterTracker_OuterTrackerTkMET_h
...
class OuterTrackerTkMET : public DQMEDAnalyzer {

public:
  explicit OuterTrackerTkMET(const edm::ParameterSet&);
  ~OuterTrackerTkMET() override;
  ...
  MonitorElement* TkMET_Loose = nullptr; // Number of tracks per event
  MonitorElement* TkMET_Med = nullptr; // Number of tracks per event
  MonitorElement* TkMET_Tight = nullptr; // Number of tracks per event
  MonitorElement* TkMET_PU = nullptr; // Number of tracks per event


 private:
  edm::ParameterSet conf_;
  edm::EDGetTokenT< l1t::VertexCollection > pvToken;
  //edm::EDGetTokenT<std::vector<TTTrack< Ref_Phase2TrackerDigi_ > > > trackToken;
