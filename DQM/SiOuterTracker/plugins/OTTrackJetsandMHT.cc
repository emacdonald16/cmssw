...
#include "DQM/SiOuterTracker/interface/OTTrackJetsandMHT.h"
...
OTTrackJetsandMHT::OTTrackJetsandMHT(const edm::ParameterSet& iConfig)
:conf_(iConfig)
{
  topFolderName_ = conf_.getParameter<std::string>("TopFolderName");

  jetToken      = consumes< l1t::L1TkJetParticleCollection > (conf_.getParameter<edm::InputTag>("L1TkJetInputTag"));


  //jetToken = consumes < std::vector< l1t::L1TkJetParticle > > (conf_.getParameter<edm::InputTag>("L1TkJetInputTag"));
...
OTTrackJetsandMHT::~OTTrackJetsandMHT()
{
...
void OTTrackJetsandMHT::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  // L1 track-trigger jets
  edm::Handle< l1t::L1TkJetParticleCollection > L1TkJetsHandle;

  //edm::Handle< std::vector< l1t::L1TkJetParticle > > L1TkJetsHandle;
  iEvent.getByToken(jetToken, L1TkJetsHandle);
  std::vector< l1t::L1TkJetParticle >::const_iterator jetIter;
  ...
  for (jetIter = L1TkJetsHandle->begin(); jetIter != L1TkJetsHandle->end(); ++jetIter) {
    ...
    float tmp_jet_ntracks = jetIter->getNtracks();
    float tmp_jet_nTighttracks = jetIter->getNTighttracks();
    float tmp_jet_nDisplacedtracks = jetIter->getNDisptracks();
    float tmp_jet_nTightDisptracks = jetIter->getNTightDisptracks();
...
void OTTrackJetsandMHT::bookHistograms(DQMStore::IBooker &iBooker, edm::Run const & run, edm::EventSetup const & es) {
...
DEFINE_FWK_MODULE(OTTrackJetsandMHT);
