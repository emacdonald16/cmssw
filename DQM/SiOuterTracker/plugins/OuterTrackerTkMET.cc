...
#include "DQM/SiOuterTracker/interface/OuterTrackerTkMET.h"
...
OuterTrackerTkMET::OuterTrackerTkMET(const edm::ParameterSet& iConfig)
:conf_(iConfig)
{
...
OuterTrackerTkMET::~OuterTrackerTkMET()
{
...
void OuterTrackerTkMET::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  // L1 Primaries
  ...
  for (auto trackIter: *TTTrackHandle) {
    ...
    float pt = tempTrackPtr->getMomentum().perp();
    float phi = tempTrackPtr->getMomentum().phi();
    float eta = tempTrackPtr->getMomentum().eta();
    ...
    float chi2dof = tempTrackPtr->getChi2() / (2*nstubs-4);
    float bendchi2 = tempTrackPtr->getStubPtConsistency();
    float z0  = tempTrackPtr->getPOCA().z();
    ...
    std::cout<<"bendchi2: "<<bendchi2<<std::endl;
...
void OuterTrackerTkMET::bookHistograms(DQMStore::IBooker &iBooker, edm::Run const & run, edm::EventSetup const & es) {
  ...
  HistoName = "TkMET_Loose";
  TkMET_Loose = iBooker.book1D(HistoName, HistoName, psTrack_TkMET.getParameter<int32_t>("Nbinsx"), psTrack_TkMET.getParameter<double>("xmin"), psTrack_TkMET.getParameter<double>("xmax"));
  TkMET_Loose->setAxisTitle("L1 Track MET (loose) [GeV]", 1);
  TkMET_Loose->setAxisTitle("# Events", 2);

  //med tkMET
  HistoName = "TkMET_Med";
  TkMET_Med = iBooker.book1D(HistoName, HistoName, psTrack_TkMET.getParameter<int32_t>("Nbinsx"), psTrack_TkMET.getParameter<double>("xmin"), psTrack_TkMET.getParameter<double>("xmax"));
  TkMET_Med->setAxisTitle("L1 Track MET (med) [GeV]", 1);
  TkMET_Med->setAxisTitle("# Events", 2);

  //tight tkMET
  HistoName = "TkMET_Tight";
  TkMET_Tight = iBooker.book1D(HistoName, HistoName, psTrack_TkMET.getParameter<int32_t>("Nbinsx"), psTrack_TkMET.getParameter<double>("xmin"), psTrack_TkMET.getParameter<double>("xmax"));
  TkMET_Tight->setAxisTitle("L1 Track MET (tight) [GeV]", 1);
  TkMET_Tight->setAxisTitle("# Events", 2);
...
DEFINE_FWK_MODULE(OuterTrackerTkMET);
