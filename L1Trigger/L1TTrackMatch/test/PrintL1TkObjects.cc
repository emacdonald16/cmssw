...
PrintL1TkObjects::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (L1TkEtMissHandle.isValid() ) {
    ...
    //std::cout << " ETmiss = " << etmiss << " for zvtx = " << zvtx << " and ETmiss from PU = " << etMissPU << std::endl;

    float etmiss = L1TkEtMissHandle -> begin() -> et();
    float etMissPU = L1TkEtMissHandle -> begin() -> etMissPU();
    std::cout << " ETmiss = " << etmiss <<" and ETmiss from PU = " << etMissPU << std::endl;
  }
